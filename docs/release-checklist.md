...
4. 公開リポジトリのmainへ反映し、`v0.2.2` タグを作成する。タグの `Release MIX-agent` が8種類のAMD64イメージをGHCRへ配布する。
5. GitHub Packagesで8種類のパッケージをPublicに変更する。匿名で各イメージをpullできることを確認する。
6. Actionsの `Publish store and GitHub Release` に `v0.2.2` を指定して実行する。生成された `store.json`、ZimaOSからの新規導入、GitHub Releaseの内容を確認する。
