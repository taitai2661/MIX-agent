...
> **開発段階に関する注意**
> MIX-agent は初期開発版です。インターネットへ直接公開したり、複数の信頼できない利用者に提供したり、未知のコードを安全に実行する目的では使用しないでください。公開環境で運用する場合は、認証、TLS、ネットワーク分離、バックアップ、ログ監視、Provider の利用規約を含む追加のセキュリティ設計が必要です。
